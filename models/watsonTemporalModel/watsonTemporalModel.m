...
%   that causes the output of the model to have a maximum value of beta
...
%                            zeta - multiplier of the amplitude of the
%                                   surround filter
...
%{
    ...
    myObj = @(p) sqrt(sum((realtiveBOLDresponse-watsonTemporalModel(stimulusFreqHz,p)).^2));
    x0 = [4 2 1 1];
    params = fmincon(myObj,x0);
    % Plot the results
    stimulusFreqHzFine = logspace(log10(0.5),log10(64),100);
    semilogx(stimulusFreqHzFine,watsonTemporalModel(stimulusFreqHzFine,params)+minBOLD,'-k');
    hold on
...
if ~isempty(params_centerAmplitude)
    ...
    params_centerAmplitudeIn = fminsearch(myObj,1,options);
    H1 = nStageLowPassFilter(params_tau,frequenciesToModel,centerFilterOrder);
    H2 = nStageLowPassFilter(params_kappa*params_tau,frequenciesToModel,surroundFilterOrder);
    y = (params_centerAmplitudeIn * H1) - (params_zeta*params_centerAmplitudeIn*H2);
end
...
% Scale the response by the beta param if we are not searching for the
% center amplitude
if isempty(params_centerAmplitude)
    y = y .* params_beta;
end
